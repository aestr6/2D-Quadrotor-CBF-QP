...
vw = 10.0;               % Vertical wind induced acceleration (applied over a time span)
hw = 3.0;                % Horizontal wind induced acceleration (constant)
...
% simulate, change hw and vw to get different plots
% (last 2 inputs are [vw, hw])
out = simulate_acc(t, x0, g, m, ca, p_sc, phi_max, vd, ti, k_p, k_d, gamma1, gamma2, beta1, beta2, pos_obs1, R_obs, vw, hw);
...
ylabel('Pitch [deg]')
title('Pitch Angle')
legend('$\phi$', '$\pm \phi_{\max}$', 'Location', 'northeast')
...
function out = simulate_acc(t, x0, g, m, ca, p_sc, phi_max, vd, ti, k_p, k_d, gamma1, gamma2, beta1, beta2, pos_obs1, R_obs, vw, hw)
    ...
    for k = 1:N-1
        ...
        uk = u(:,k);
        dyn = @(tau, xk) acc_rhs(xk, uk, g, m, t(k), vw, hw);
...
function [u, h_obs, h_obs_ineq, h_y_ineq, exitflag] = acc_qp(opts_qp, t_k, x, x0, g, m, ca, p_sc, phi_max, vd, ti, k_p, k_d, gamma1, gamma2, beta1, beta2, pos_obs1, R_obs)
...
function dx = acc_rhs(x, u, g, m, t, vw, hw)
    ts = 6.0;
    tf = 8.0;
    if (ts < t && t < tf)
        dx = [x(3); x(4); u(1)/m + hw; u(2)/m-g - vw];
    else
        dx = [x(3); x(4); u(1)/m + hw; u(2)/m-g];
    end
end
