...
% simulate
out = simulate_acc(t, x0, g, m, ca, p_sc, phi_max, vd, ti, k_p, k_d, gamma1, gamma2, beta1, beta2, pos_obs1, R_obs);

out_nom = simulate_nom(t, x0, g, m, vd, ti, k_p, k_d);
...
plot(out_nom.x(1, :), out_nom.x(2, :), 'r-.', 'LineWidth', 2, 'DisplayName', 'Nominal (No CBF)');
...
function out = simulate_acc(t, x0, g, m, ca, p_sc, phi_max, vd, ti, k_p, k_d, gamma1, gamma2, beta1, beta2, pos_obs1, R_obs)
...
function out_nom = simulate_nom(t, x0, g, m, vd, ti, k_p, k_d)
    N = numel(t);
    x = zeros(4, N);
    u = zeros(2, N);
    x(:, 1) = x0;
    ...
    for k = 1:N-1
        [x_des, dx_des] = des_traj(x0, t(k), vd, ti);
        
        u_x_nom = m * (-k_p*(x(1, k) - x_des(1)) - k_d*(x(3, k) - dx_des(1)));
        u_y_nom = m * (-k_p*(x(2, k) - x_des(2)) - k_d*(x(4, k) - dx_des(2)) + g);
        u(:, k) = [u_x_nom; u_y_nom];
